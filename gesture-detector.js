...
AFRAME.registerComponent("gesture-detector", {
  ...
  init: function () {
    // Set target element for gesture detection
  ...
  getTouchState(event) {
    ...
    touchState.positionRaw = { x: centerX, y: centerY };

    // Normalize position to screen size
    const screenScale = 2 / (window.innerWidth + window.innerHeight);
    touchState.position = { x: centerX * screenScale, y: centerY * screenScale };
  ...
  getEventPrefix(touchCount) {
    return ["one", "two", "three", "many"][Math.min(touchCount, 4) - 1];
  }
});
